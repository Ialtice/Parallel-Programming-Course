...
int main(int argc, char *argv[]) {
    ...
    unsigned char *color_img,*gray_img, *color_img_tmp, *gray_img_tmp;
    //1
    if(rank == 0){
...
//4
    size_t gray_img_tmp_size = width * ceil(height / p ) * gray_channels;
    size_t color_img_tmp_size = width * ceil(height / p ) * channels;
    color_img_tmp = (unsigned char *)malloc(color_img_tmp_size);
    gray_img_tmp = (unsigned char *)malloc(gray_img_tmp_size);
//5
    MPI_Scatter(color_img,color_img_tmp_size,MPI_CHAR, color_img_tmp,color_img_tmp_size, MPI_CHAR,0,MPI_COMM_WORLD);
    //6
    colorToGrayscale(gray_img_tmp, color_img_tmp, width, ceil(height/p));
    //7
    MPI_Gather(gray_img_tmp,gray_img_tmp_size, MPI_CHAR, gray_img, gray_img_tmp_size, MPI_CHAR,0,MPI_COMM_WORLD);
//8
    if(rank == 0){
    	time_t end = clock();
    	printf("Color to gray scale conversion in parallel takes %lld clicks (%f milliseconds)\n", (end- start),((end-start)*1000/(CLOCKS_PER_SEC + 0.0)));
    	if (type == IS_PNG)
